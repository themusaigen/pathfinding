...
function AstarConfiguration.new()
  ...
  local default = {
    ...
    collision = function(self, target, origin)
      return not processLineOfSight(origin.x, origin.y, origin.z, target.x, target.y, target.z,
        self.check_buildings, self.check_vehicles, self.check_peds, self.check_objects, self.check_particles,
        self.check_transparent_objects, self.ignore_dynamic_objects, self.check_shootable_objects)
    end,
